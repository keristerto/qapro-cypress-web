...
describe('check_out', () => {
    ...
    it("Deve exibir uma mensagem. ao clicar en Place order sem preencher os datos de pagamento", () => {
        cy.get(':nth-child(2) > :nth-child(2) > .theme-btn-one')
            .click()
        cy.get('.payment_method > #errorMessageFirstName')
            .should('be.visible')
            .should('have.text', 'Preencha os dados de pagamento!')
    })

    it.only("Finalizar compra", () => {
        cy.get('.form-control').eq(0)
            .type('keris')
        cy.get('.form-control').eq(1)
            .type('Terto')
        ...
        cy.get('.offer_modal_left')
            .should('be.visible')
            .should('have.text', 'Order success!Congrats! Your order was created with sucess!')
    })
